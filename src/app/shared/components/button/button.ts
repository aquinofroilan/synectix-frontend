            ...
            @if (isLoading) {
                <ng-container>
                    <span class="flex items-center">
                        <svg class="w-4 h-4 mr-2 animate-spin" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                ...
                <ng-template>
                    <span class="flex items-center">
                        ...
                        @if (!iconOnly) {
                            <span [ngClass]="{ 'text-gray-400 hover:underline-offset-2': variant === 'link' }">{{
                                label
                            }}</span>
                        }
                    </span>
...
export class Button {
    ...
    get computedClass(): string {
        const base = this.customClass + " cursor-pointer";
        return this.disabled || this.isLoading ? `opacity-50 cursor-not-allowed ${base}` : base;
    }
