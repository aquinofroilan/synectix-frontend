    ...
    template: `
        @if (asAnchor) {
            <a
                ...
                (click)="onDisabledChange(disabled)"
                >{{ label }}
                @if (withIcon) {
...
export class Button {
    ...
    protected onDisabledChange(isDisabled: boolean): void {
        ...
        this.customClass = isDisabled ? "opacity-50 cursor-not-allowed " + this.customClass : this.customClass;
    }

    protected onLoadingChange(isLoading: boolean): void {
        this.isLoading = isLoading;
        this.customClass = isLoading ? "opacity-50 cursor-not-allowed " + this.customClass : this.customClass;
    }
}
